    ...
    <div class="space-y-4">
      ...
      >
        ...
        <button
          ...
          class="w-full p-6 hover:bg-muted/50 transition-colors rounded-t-lg"
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              <ChevronDown v-if="expandedBatches.has(batch.id)" class="h-4 w-4 text-muted-foreground" />
              <ChevronRight v-else class="h-4 w-4 text-muted-foreground" />
              <div class="text-left">
                <h4 class="text-base font-semibold">{{ batch.name }}</h4>
                <p class="text-sm text-muted-foreground">{{ batch.date }}</p>
              </div>
            </div>
            <div class="flex items-center gap-3">
        ...
        <!-- Expanded Content -->
        <div v-if="expandedBatches.has(batch.id)" class="border-t p-6 pt-0">
            ...
            <table class="w-full">
              <thead>
                <tr class="border-b bg-muted/50">
                  ...
                  <th class="h-10 px-4 text-left align-middle font-medium text-muted-foreground text-sm">Precision</th>
                  <th class="h-10 px-4 text-left align-middle font-medium text-muted-foreground text-sm">Recall</th>
                  <th class="h-10 px-4 text-left align-middle font-medium text-muted-foreground text-sm">F1-Score</th>
                  <th class="h-10 px-4 text-left align-middle font-medium text-muted-foreground text-sm">Actions</th>
                </tr>
              </thead>
              <tbody>
                <tr 
                  v-for="experiment in batch.experiments" 
                  :key="experiment.id" 
                  class="border-b hover:bg-muted/50 transition-colors"
                >
                  <td class="p-4 align-middle">
                    <div>
                      <div class="text-xs text-muted-foreground font-mono">{{ experiment.id }}</div>
                      <div class="text-sm">{{ experiment.name }}</div>
                    </div>
                  </td>
                  <td class="p-4 align-middle">
                  ...
                  <td class="p-4 align-middle">
                    <div class="flex items-center gap-2">
                      <span class="font-mono text-sm">{{ experiment.auc.toFixed(3) }}</span>
                  ...
                  <td class="p-4 align-middle">
                    <div class="flex items-center gap-2">
                      <span class="font-mono text-sm">{{ experiment.precision.toFixed(3) }}</span>
                      <div class="w-12 h-1 bg-secondary rounded-full overflow-hidden">
                        <div 
                          class="h-full bg-primary transition-all duration-500"
                  ...
                  <td class="p-4 align-middle">
                    <div class="flex items-center gap-2">
                      <span class="font-mono text-sm">{{ experiment.recall.toFixed(3) }}</span>
                      <div class="w-12 h-1 bg-secondary rounded-full overflow-hidden">
                        <div 
                          class="h-full bg-primary transition-all duration-500"
                  ...
                  <td class="p-4 align-middle">
                    <div class="flex items-center gap-2">
                      <span class="font-mono text-sm">{{ experiment.f1.toFixed(3) }}</span>
                      <div class="w-12 h-1 bg-secondary rounded-full overflow-hidden">
                        <div 
                          class="h-full bg-primary transition-all duration-500"
                  ...
                  <td class="p-4 align-middle">
                    <button
                      @click.stop="$emit('experiment-click', experiment.id)"
...
const expandedBatches = ref<Set<string>>(new Set(['batch_2024_01_15']))

const toggleBatch = (batchId: string) => {
  const newExpanded = new Set(expandedBatches.value)
  if (newExpanded.has(batchId)) {
    newExpanded.delete(batchId)
  } else {
    newExpanded.add(batchId)
  }
  expandedBatches.value = newExpanded
}
